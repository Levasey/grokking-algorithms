...
public class DistributedAlgorithmsTest {
    ...
    public void vectorClock_detectsConcurrent() {
        ...
        a.tick();
        b.tick();
        assertEquals(VectorClock.CausalRelation.CONCURRENT, VectorClock.relation(a.snapshot(), b.snapshot()));
    }
    ...
    public void vectorClock_happensBefore_acrossMessage() {
        ...
        int[] m = p0.onSend();
        p1.onReceive(m);
        assertEquals(VectorClock.CausalRelation.BEFORE, VectorClock.relation(p0.snapshot(), p1.snapshot()));
        assertEquals(VectorClock.CausalRelation.AFTER, VectorClock.relation(p1.snapshot(), p0.snapshot()));
    }
    ...
    public void bully_trace_fromSmallest() {
        List<BullyLeaderElection.ElectionStep> trace = new ArrayList<>();
