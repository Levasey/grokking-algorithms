 ...
 * <p>
 * Для каждой функции берётся минимум {@code a·feat + b} с сравнением как беззнаковое {@link Long}
 * ({@link Long#compareUnsigned}), чтобы переполнение 64-битового произведения не ломало порядок.
 * Сложность подписи — {@code O(число_функций × |признаки|)}.
 */
