 ...
 * <p>
 * Хеширование учебное (не криптостойкое). Экземпляр не потокобезопасен. Вероятность из конструктора
 * {@link #BloomFilter(int, double)} — модельная оценка при предположении «хорошего» распределения битов, не аудит для атак.
 */
public final class BloomFilter {
    ...
    /**
     * Добавляет элемент в множество. Один и тот же элемент можно вызывать повторно (идемпотентно по битам).
     */
